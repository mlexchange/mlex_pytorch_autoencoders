...
if __name__ == "__main__":
    ...
    [test_loader, temp], (temp_channels, temp_w, temp_h), tmp = get_dataloaders(
        args.input_dir,
        eval_parameters.batch_size,
        NUM_WORKERS,
        False,
    ...
    for count, latent_dim in enumerate(eval_parameters.latent_dim):
        with io.capture_output() as captured:
            trainer = pl.Trainer(
                default_root_dir=args.output_dir + f"/model_{latent_dim}",
                ...
                callbacks=[
                    ModelCheckpoint(
                        dirpath=args.output_dir + f"/model_{latent_dim}",
                        save_last=True,
            ...
            model_ld = Autoencoder(
                base_channel_size=eval_parameters.base_channel_size,
            ...
            test_result = trainer.test(
                model_ld, test_dataloaders=test_loader, verbose=False
            )
            if val_loader:
                val_result = trainer.test(
                    model_ld, test_dataloaders=val_loader, verbose=False
                )
        if count == 0:
